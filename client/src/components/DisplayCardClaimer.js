...
const DisplayCardClaimer = ({ claimant, number }) => {
  ...
  // You can style the card as needed
  const cardStyle = {
    backgroundColor: '#e6f7ff', // Bluish tint
    padding: '10px',
    margin: '10px',
    borderRadius: '8px',
  ...
  return (
    <div style={cardStyle}>
      ...
      <p><strong>Item Claimed:</strong> {itemdetails}</p>
      <p><strong>Mobile Number:</strong> {mobilenumber}</p>
      <p><strong>Hostel Name:</strong> {hostelname}</p>
      <p><strong>Proof of Claim:</strong> {proofofclaim}</p>
